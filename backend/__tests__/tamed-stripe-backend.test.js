...
test('generateProduct', async () => {
	let now = Date.now();
	let name = `Product ${now}`;
	let description = `Generated by JEST tests. Product ${now}`;
	let currency = 'usd';
	let unitAmountDecimal = "100001";
	let props = {
		name,
		description,
		currency,
	...
	let productData = response.payload;
...
test('generateCheckoutForSubscription', async () => {
	...
	let subscriptionResponse = await tsb.generateCheckoutForSubscription({
	...
	tickLog.info(`subscriptionResponse: ${JSON.stringify(subscriptionResponse, null, 2)}`); // deleteme
